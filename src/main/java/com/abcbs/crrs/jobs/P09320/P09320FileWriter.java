...
public class P09320FileWriter implements ItemWriter<P09320Record>, ItemStream {

	private static final Logger log = LogManager.getLogger(P09320FileWriter.class);
	...
	public P09320FileWriter(String reportPath, IActivityRepository activityRepository,
			IP09CashReceiptRepository receiptRepository, IP09ControlRepository controlRepository,P09TableLocker tableLocker) {
	...
	// ItemWriter
	@Override
	public void write(Chunk<? extends P09320Record> items) throws Exception {
		for (P09320Record input : items) {
			try {
	...
	@Transactional
	private void executeJob(P09320Record input) throws Exception {
		log.info("=== START P09320 job for refundType: {} ===", input.getRefundType());
		P09320Vars ws = new P09320Vars();
		...
		log.info("=== END P09320 job for refundType: {} ===", input.getRefundType());
	}

	private void initialization(P09320Record input, P09320Vars ws) {
	...
	private void getControlRecord(P09320Vars ws) {
	...
	private void getCrEstCntsAmts(P09320Vars ws) {
		String toDate = ws.rptDate;
		log.debug("Querying EST activity/receipts for refundType={} toDate={}", ws.getRefundType(), toDate);
		List<Object[]> rows;
		try {
			rows = activityRepository.findActivityReceiptRecords(ws.getRefundType(), List.of("EST"), toDate);
		} catch (DataAccessException dae) {
	...
	private void getResolvedCr(P09320Vars ws) {
		String toDate = ws.rptDate;
		log.debug("Querying resolved activities for refundType={} toDate={}", ws.getRefundType(), toDate);

		List<Object[]> rows;
		try {
			rows = activityRepository.findActivityReceiptRecordsOrdered(ws.getRefundType(),
	...
	private void evalClaimType(P09320Vars ws, P09CashReceipt rec, int wsCount, BigDecimal amt) {
		String raw = rec.getCrClaimType() == null ? "99" : rec.getCrClaimType().trim();
		String key = P09320FileWriter.CLAIM_MAP.getOrDefault(raw, "99");
	...
	private void calcCrEndingBal(P09320Vars ws) {
	...
	private void getAllOpenPendedCr(P09320Vars ws) {
		...
		try {
			recs = receiptRepository.findOpenOrPendedReceipts(ws.getRefundType(), toDate);
		} catch (DataAccessException dae) {
		...
		for (P09CashReceipt r : recs) {
			...
			String key = P09320FileWriter.CLAIM_MAP.getOrDefault(raw, "99");
			P09320Vars.OpenBucket bucket = ws.openBuckets.get(key);
			if (bucket == null) {
				bucket = new P09320Vars.OpenBucket();
	...
	private void calcOpenTotals(P09320Vars ws) {
		...
		for (Map.Entry<String, P09320Vars.OpenBucket> e : ws.openBuckets.entrySet()) {
			P09320Vars.OpenBucket b = e.getValue();
			totReqCnt += b.reqCnt;
	...
	private void calcTotalAging(P09320Vars ws) {
	...
	private void calcResolvedTotals(P09320Vars ws) {
	...
	private void calcMonthlyAccrual(P09320Vars ws) {
	...
	private void generateDetailLines(P09320Vars ws) throws IOException {
		...
		java.util.function.Consumer<Integer> writeHeader = (pg) -> {
			try {
				...
				String line3 = padRight("", 48) + padRight("FOR REFUND TYPE: ", 17) + padRight(ws.getRefundType(), 3)
						+ padRight(" - ", 3) + padRight(ws.ctlNarr, 13);
		...
		reportWriter.write(padRight("", 26) + padRight("CASH RECEIPTS BEGINNING", 23) + padRight("", 1)
				+ padRight("BALANCE", 7) + padRight("", 2) + padRight(repeat('.', 25), 25) + padRight("", 3)
				+ fmtCount.apply(ws.ctlReceiptCnt) + padLeft("", 2) + fmtAmt.apply(ws.ctlReceiptAmt));
		reportWriter.newLine();
		reportWriter.newLine();
		...
		reportWriter.write(padRight("", 26) + padRight("CASH RECEIPTS ENDING BALANCE", 28) + padRight("", 2)
				+ padRight(repeat('.', 28), 28) + "   " + fmtCount.apply(ws.wsCrEndBalCnt) + "  "
				+ fmtAmt.apply(ws.wsCrEndBalAmt));
		reportWriter.newLine();
		if (ws.getRefundType().equals("PER")) {
			...
			// TO MEDIPAK
			reportWriter.write(
					padRight("", 31) + padRight("TO MEDIPAK", 10) + padRight("", 28) + fmtAmt.apply(ws.wsMedipakAmt));
			reportWriter.newLine();

			// TO OTHER
			reportWriter
					.write(padRight("", 31) + padRight("TO OTHER", 8) + padRight("", 30) + fmtAmt.apply(ws.wsOtherAmt));
			reportWriter.newLine();
		...
		for (String code : claimCodes) {
			...
			P09320Vars.OpenBucket ob = ws.openBuckets.get(code);
		...
		reportWriter.write(repeat('*', 59) + "    END OF REPORT P09320-A    " + repeat('*', 44));
		reportWriter.newLine();
		reportWriter.flush();
		log.info("Report generation complete for refundType={}", ws.getRefundType());
	}

	private void finalizeControlRecord(P09320Vars ws) {
	...
	private void initOpenBuckets(P09320Vars ws) {
		// initialize 01..24 and 99
		for (int i = 1; i <= 24; i++) {
			ws.openBuckets.put(String.format("%02d", i), new P09320Vars.OpenBucket());
		}
		ws.openBuckets.put("99", new P09320Vars.OpenBucket());
	}
