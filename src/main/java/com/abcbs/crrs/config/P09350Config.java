...
import com.abcbs.crrs.jobs.P09350.ChkpCardRec;
import com.abcbs.crrs.jobs.P09350.InputVoucherRec;
import com.abcbs.crrs.jobs.P09350.P09350ApInterfaceOutput;
import com.abcbs.crrs.jobs.P09350.P09350ControlCardInput;
import com.abcbs.crrs.jobs.P09350.P09350CorpCardInput;
import com.abcbs.crrs.jobs.P09350.P09350LineMappers;
import com.abcbs.crrs.jobs.P09350.P09350OutputVoucher;
import com.abcbs.crrs.jobs.P09350.P09350OutputWrapper;
import com.abcbs.crrs.jobs.P09350.P09350Processor;
import com.abcbs.crrs.jobs.P09350.P09350ReportWriter;
import com.abcbs.crrs.jobs.P09350.P09350XP07DedsOutput;
import com.abcbs.crrs.jobs.P09350.P09350XP09DedsOutput;

@Configuration
public class P09350Config {
	@Bean
    public Job p09350Job(JobRepository jobRepository,
                         @Qualifier("p09350ProcessStep") Step p09350ProcessStep,
                         @Qualifier("p09350VoucherStep") Step p09350VoucherStep) {

        return new JobBuilder("P09350Job", jobRepository)
                .start(p09350ProcessStep)
                .next(p09350VoucherStep)
                .build();
    }

	
	@Bean(name = "p09350ProcessStep")
    public Step p09350ProcessStep(JobRepository repo,
                                  PlatformTransactionManager transactionManager,
                                  @Qualifier("controlCardReader350") org.springframework.batch.item.ItemReader<P09350ControlCardInput> controlReader350,
                                  @Qualifier("corpCardReader350") FlatFileItemReader<P09350CorpCardInput> corpReader350,
                                  @Qualifier("checkpointReader350") FlatFileItemReader<ChkpCardRec> checkpointReader350,
                                  @Qualifier("inputVoucherReader350") FlatFileItemReader<InputVoucherRec> inputVoucherReader350,
                                  P09350Processor processor,
                                  P09350ReportWriter reportWriter,
                                  @Qualifier("apWriter350") FlatFileItemWriter<P09350ApInterfaceOutput> apWriter350,
                                  @Qualifier("glWriter350") FlatFileItemWriter<P09350XP09DedsOutput> glWriter350,
                                  @Qualifier("occsWriter350") FlatFileItemWriter<P09350XP07DedsOutput> occsWriter350,
                                  ExecutionContextPromotionListener p09350PromotionListener) {

    	return new StepBuilder("p09350ProcessStep", repo)
    		    .listener(reportWriter)              // keep ONE
    		    .listener(p09350PromotionListener)
    		    ...
    		    .tasklet((contribution, chunkContext) -> {
    		        ...
    		        corpReader350.open(stepEc);
    		        checkpointReader350.open(stepEc);
    		        inputVoucherReader350.open(stepEc);

    		        apWriter350.open(stepEc);
    		        glWriter350.open(stepEc);
    		        occsWriter350.open(stepEc);

    		        try {
    		        	P09350CorpCardInput corpCardrec=corpReader350.read();

    		            P09350OutputWrapper out = processor.process(corpCardrec);
    		            ...
    		            if (apCnt > 0) apWriter350.write(new Chunk<>(out.getApRecords()));
    		            if (glCnt > 0) glWriter350.write(new Chunk<>(out.getGlRecords()));
    		            if (ocCnt > 0) occsWriter350.write(new Chunk<>(out.getOccsRecords()));

    		            return org.springframework.batch.repeat.RepeatStatus.FINISHED;
    		        } finally {
    		            try { occsWriter350.close(); } catch (Exception ignore) {}
    		            try { glWriter350.close(); } catch (Exception ignore) {}
    		            try { apWriter350.close(); } catch (Exception ignore) {}

    		            try { inputVoucherReader350.close(); } catch (Exception ignore) {}
    		            try { checkpointReader350.close(); } catch (Exception ignore) {}
    		            try { corpReader350.close(); } catch (Exception ignore) {}    		           
    		        }
    		    }, transactionManager)
    		    .build();
    }
	
	@Bean(name = "p09350VoucherStep")
    public Step p09350VoucherStep(JobRepository repo,
                                  PlatformTransactionManager transactionManager,
                                  @Qualifier("voucherWriter350") FlatFileItemWriter<P09350OutputVoucher> voucherWriter) {

    	return new StepBuilder("p09350VoucherStep", repo)
    		    .tasklet((contribution, chunkContext) -> {
    		        ...
    		        P09350OutputVoucher out = new P09350OutputVoucher();
	...
	@Bean(name = "corpCardReader350")
    @StepScope
    public FlatFileItemReader<P09350CorpCardInput> corpCardReader350(
            @Value("#{jobParameters['CORP_CARD']}") String CORP_CARD) {

        return new FlatFileItemReaderBuilder<P09350CorpCardInput>()
                .name("corpCardReader350")
                .resource(new FileSystemResource(CORP_CARD))
                .lineMapper(P09350LineMappers.corpCardLineMapper())
                .build();
    }
	
	@Bean(name = "checkpointReader350")
    @StepScope
    public FlatFileItemReader<ChkpCardRec> checkpointReader350(
            @Value("#{jobParameters['CHKP_CARD']}") String CHKP_CARD) {

        return new FlatFileItemReaderBuilder<ChkpCardRec>()
                .name("checkpointReader350")
                .resource(new FileSystemResource(CHKP_CARD))
                .lineMapper(P09350LineMappers.checkpointLineMapper())
                .build();
    }
	
	@Bean(name = "inputVoucherReader350")
    @StepScope
    public FlatFileItemReader<InputVoucherRec> inputVoucherReader350(
            @Value("#{jobParameters['INPUT_VOUCHER_NBR']}") String ivoucherFile) {

        return new FlatFileItemReaderBuilder<InputVoucherRec>()
                .name("inputVoucherReader350")
                .resource(new FileSystemResource(ivoucherFile))
                .lineMapper(P09350LineMappers.inputVoucherLineMapper())
                .build();
    }
	
	@Bean(name = "controlCardReader350")
    @org.springframework.batch.core.configuration.annotation.StepScope
    public org.springframework.batch.item.ItemReader<P09350ControlCardInput> controlCardReader350() {
        ...
        return new org.springframework.batch.item.support.ListItemReader<>(
                java.util.Collections.singletonList(new P09350ControlCardInput())
        );
    }
     ...
     */
    @Bean(name = "apWriter350")
    @StepScope
    public FlatFileItemWriter<P09350ApInterfaceOutput> apWriter350(
            @Value("#{jobParameters['AP_INTRFACE_OUT']}") String xapntrfcPath) {

        LineAggregator<P09350ApInterfaceOutput> agg = item -> {
            String invoiceDate =
        ...
        return new FlatFileItemWriterBuilder<P09350ApInterfaceOutput>()
                .name("apWriter")
    ...
    @Bean(name = "glWriter350")
    @StepScope
    public FlatFileItemWriter<P09350XP09DedsOutput> glWriter(
            @Value("#{jobParameters['GENERAL_LEDGER_OUT']}") String xp09Path) {

        // Leaving your existing field widths, but safer “new file per run”
        org.springframework.batch.item.file.transform.FormatterLineAggregator<P09350XP09DedsOutput> agg =
        ...
        return new FlatFileItemWriterBuilder<P09350XP09DedsOutput>()
                .name("glWriter")
    ...
    @Bean(name = "occsWriter350")
    @org.springframework.batch.core.configuration.annotation.StepScope
    public FlatFileItemWriter<P09350XP07DedsOutput> occsWriter(
            @Value("#{jobParameters['OCCS_DEDS_FILE_OUT']}") String xp07Path) {

        org.springframework.batch.item.file.transform.FormatterLineAggregator<P09350XP07DedsOutput> agg =
                new org.springframework.batch.item.file.transform.FormatterLineAggregator<>();
        ...
        return new FlatFileItemWriterBuilder<P09350XP07DedsOutput>()
                .name("occsWriter")
    ...
    @Bean(name = "voucherWriter350")
    @StepScope
    public FlatFileItemWriter<P09350OutputVoucher> voucherWriter350(
            @Value("#{jobParameters['OUTPUT_VOUCHER_NBR']}") String xvoucherPath) {

        LineAggregator<P09350OutputVoucher> agg = item ->
        ...
        return new FlatFileItemWriterBuilder<P09350OutputVoucher>()
                .name("voucherWriter")
